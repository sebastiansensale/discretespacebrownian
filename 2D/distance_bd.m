...
for factor=1:1
    trajectories=500;D=1;delta=1/5;limit=0.2;separation=1;
    for i=1:trajectories
        ...
        while nostop
            j=j+1;
            [dtA,probA]=timecont(D,delta,coordA(j-1,:),0);
            ...
            clearvars -except j timeA timeB coordA coordB i trajectories nostop D delta limit endtime  TIM factor separation
        end;
        
        TIM(i)=endtime;
        clearvars -except TIM i trajectories factor D delta limit separation
    end;
    Tim=TIM(TIM~=0);
